...
const signUp = async (req, res, next) => { // signUp 하는 로직
  try {
    ...
    const user = await User.findOne({ 'email':email }); // email 의 정보를 가지고 Users 콜렉션에서 조회한다.
...
const createToken = (userId) => {
    const token = jwt.sign({ _id: userId.toString() }, process.env.SECRET_KEY); // 고유 id로 토큰 제작. 두번째 인자값은 salt 값으로 보안과 관련된 값, 보통 dotenv 로 환경변수에 넣어서 사용함.
    return token;
};
...
const signIn = async (req, res, next) => {
    try {
      ...
      const { email, password } = req.query; // GET 메소드로 들어온 요청의 데이터(query)에서 email, password 를 destructuring 한다.
      ...
      res.status(201).json({ 'message': "Welcome", 'email':email, token }); // token 을 response로 넘겨준다.
      console.log(token);
    } catch (err) {
      next(err);
    }
